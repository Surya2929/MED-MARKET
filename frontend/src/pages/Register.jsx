...
import { UserPlus, User, Store, ShieldCheck, Mail, Lock, Phone, UserCircle, Eye, EyeOff } from 'lucide-react'; // 🚀 FIX: IMPORTED EYE ICONS
  ...
  return (
    <div className="flex min-h-[90vh] bg-slate-200 flex-row-reverse">
      ...
      <div className="hidden lg:flex w-5/12 relative bg-teal-900">
        <img src="https://images.unsplash.com/photo-1585435557343-3b092031a831?q=80&w=2070&auto=format&fit=crop" alt="Pharmacy Details" className="absolute inset-0 w-full h-full object-cover mix-blend-overlay opacity-80" />
        ...
        <div className="w-full max-w-2xl bg-slate-100 p-8 sm:p-12 rounded-3xl shadow-xl border border-slate-300">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex p-1 bg-slate-300 rounded-xl mb-6">
              <button type="button" onClick={() => setRole('customer')} className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-lg transition-all ${role === 'customer' ? 'bg-slate-100 text-blue-700 shadow-sm border border-slate-300' : 'text-slate-600 hover:text-slate-800'}`}><User className="w-5 h-5" /> I am a Customer</button>
              <button type="button" onClick={() => setRole('vendor')} className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-lg transition-all ${role === 'vendor' ? 'bg-slate-100 text-teal-700 shadow-sm border border-slate-300' : 'text-slate-600 hover:text-slate-800'}`}><Store className="w-5 h-5" /> I am a Pharmacy</button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="tel" required value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value.replace(/\D/g, '').slice(0, 10) })} placeholder="9876543210" className="w-full pl-[5.5rem] pr-4 py-3.5 bg-slate-200 border border-slate-300 rounded-xl text-slate-900 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20 transition-all text-lg placeholder-slate-500" />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? "text" : "password"} required value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder="Create a strong password" className="w-full pl-12 pr-12 py-3.5 bg-slate-200 border border-slate-300 rounded-xl text-slate-900 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20 transition-all text-lg placeholder-slate-500" />
            ...
            {role === 'vendor' && (
              <div className="mt-6 p-6 bg-teal-100/50 border border-teal-200 rounded-2xl space-y-4">
                ...
                <input type="text" required value={storeData.storeName} onChange={(e) => setStoreData({ ...storeData, storeName: e.target.value })} placeholder="Store Name" className="w-full px-4 py-3.5 bg-slate-200 border border-teal-300 rounded-xl text-slate-900 outline-none focus:border-teal-600 focus:ring-4 focus:ring-teal-600/20 transition-all text-lg placeholder-slate-500" />
                <input type="text" required value={storeData.address} onChange={(e) => setStoreData({ ...storeData, address: e.target.value })} placeholder="Full Address" className="w-full px-4 py-3.5 bg-slate-200 border border-teal-300 rounded-xl text-slate-900 outline-none focus:border-teal-600 focus:ring-4 focus:ring-teal-600/20 transition-all text-lg placeholder-slate-500" />
                <input type="text" required value={storeData.licenseNumber} onChange={(e) => setStoreData({ ...storeData, licenseNumber: e.target.value })} placeholder="Drug License Number" className="w-full px-4 py-3.5 bg-slate-200 border border-teal-300 rounded-xl text-slate-900 outline-none focus:border-teal-600 focus:ring-4 focus:ring-teal-600/20 transition-all text-lg placeholder-slate-500" />
              </div>
            )}
