    ...
    <div className="flex min-h-[90vh] bg-slate-200 flex-row-reverse">
      <div className="hidden lg:flex w-5/12 relative bg-teal-900">
        ...
        <div className="relative z-10 flex flex-col justify-center p-12 text-slate-100 w-full h-full">
          <h2 className="text-4xl font-bold leading-tight mb-4 mt-auto">Join the Healthiest Network.</h2>
          <p className="text-lg text-teal-100">Sign up to order medicines safely or register your pharmacy to reach more customers.</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-7/12 flex items-center justify-center p-6 sm:p-12 bg-slate-300/30">
        <div className="w-full max-w-2xl bg-slate-100 p-8 sm:p-12 rounded-3xl shadow-xl border border-slate-300">
          <div className="mb-8"><h2 className="text-3xl font-bold text-slate-800">Create an Account</h2><p className="text-slate-600 mt-2 text-lg">It takes less than a minute.</p></div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex p-1 bg-slate-300 rounded-xl mb-6">
              <button type="button" onClick={() => setRole('customer')} className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-lg transition-all ${role === 'customer' ? 'bg-slate-100 text-blue-700 shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}><User className="w-5 h-5" /> I am a Customer</button>
              <button type="button" onClick={() => setRole('vendor')} className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-lg transition-all ${role === 'vendor' ? 'bg-slate-100 text-teal-700 shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}><Store className="w-5 h-5" /> I am a Pharmacy</button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="relative"><UserCircle className="absolute left-4 top-3.5 w-5 h-5 text-slate-500" /><input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Full Name" className="w-full pl-12 pr-4 py-3.5 bg-slate-200 border border-slate-300 rounded-xl text-slate-900 outline-none focus:border-blue-500" /></div>
            ...
            <div className="relative"><Mail className="absolute left-4 top-3.5 w-5 h-5 text-slate-500" /><input type="email" required value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder="Email Address" className="w-full pl-12 pr-4 py-3.5 bg-slate-200 border border-slate-300 rounded-xl text-slate-900 outline-none focus:border-blue-500" /></div>
            <div className="relative"><Lock className="absolute left-4 top-3.5 w-5 h-5 text-slate-500" /><input type={showPassword ? "text" : "password"} required value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder="Create Password" className="w-full pl-12 pr-12 py-3.5 bg-slate-200 border border-slate-300 rounded-xl text-slate-900 outline-none focus:border-blue-500" /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-500 hover:text-slate-700">{showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}</button></div>

            {role === 'vendor' && (
              <div className="mt-6 p-6 bg-teal-100/50 border border-teal-200 rounded-2xl space-y-4">
                <h3 className="font-bold text-teal-800 flex items-center gap-2 mb-4 text-lg"><ShieldCheck className="w-6 h-6" /> Pharmacy Details</h3>
                
                {/* 🚀 NEW: ONLINE OR LOCAL VENDOR SELECTION */}
                <div className="flex gap-4 mb-4">
                  <label className={`flex-1 flex items-center justify-center gap-2 p-3 border rounded-xl cursor-pointer transition-all ${storeData.storeType === 'offline' ? 'bg-white border-teal-500 text-teal-800 shadow-sm' : 'border-slate-300 text-slate-500'}`}>
                    ...
                    <MapPin className="w-4 h-4"/> Local Store
                  </label>
                  <label className={`flex-1 flex items-center justify-center gap-2 p-3 border rounded-xl cursor-pointer transition-all ${storeData.storeType === 'online' ? 'bg-white border-teal-500 text-teal-800 shadow-sm' : 'border-slate-300 text-slate-500'}`}>
                    ...
                    <Globe className="w-4 h-4"/> National (Online)
                  </label>
                </div>
                
                <input type="text" required value={storeData.storeName} onChange={(e) => setStoreData({ ...storeData, storeName: e.target.value })} placeholder="Store Name" className="w-full px-4 py-3.5 bg-slate-200 rounded-xl outline-none focus:border-teal-600" />
                <input type="text" required value={storeData.address} onChange={(e) => setStoreData({ ...storeData, address: e.target.value })} placeholder={storeData.storeType === 'offline' ? "Full Local Address & City" : "Warehouse Address"} className="w-full px-4 py-3.5 bg-slate-200 rounded-xl outline-none focus:border-teal-600" />
                <input type="text" required value={storeData.licenseNumber} onChange={(e) => setStoreData({ ...storeData, licenseNumber: e.target.value })} placeholder="Drug License Number" className="w-full px-4 py-3.5 bg-slate-200 rounded-xl outline-none focus:border-teal-600" />
              </div>
            )}
          ...
          <p className="text-center text-slate-600 text-lg mt-8 border-t border-slate-300 pt-6">
            Already have an account? <Link to="/login" className="font-bold text-blue-700 hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
